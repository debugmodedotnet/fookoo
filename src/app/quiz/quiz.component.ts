...
import { QuizUpdatedService } from '../services/quiz-updated.service';
...
export class QuizComponent implements OnInit {
  ...
  private quizupdateservice = inject(QuizUpdatedService);
  ...
  // }
   ...
   ngOnInit() {
    this.route.params.subscribe((params) => {
      this.technologyName = params['Name'];
      this.quizupdateservice.createQuiz({ technologyName: this.technologyName }).subscribe((res:any)=>{
        console.log(res); 
      });
  ...
  async loadTechnologyData(): Promise<void> {
    this.quizService
      .getTechnology(this.technologyName)
      .pipe(first())
      .subscribe(async (res) => {
        this.technology = res;
        if (this.technology) {
          this.totalQuestions =
            (await this.quizService
              .getTotalQuestions(this.technologyName)
              .toPromise()) ?? 0;
          this.loadNewQuestion();
        }
      });
  ...
  getQuestion(questionIdsToExclude: string[]): void {
    this.quizService
      .getQuestion(this.technologyName, questionIdsToExclude)
      .pipe(first())
      .subscribe((firstRes) => {
        if (firstRes.length) {
          ...
          this.quizService
            .getQuestion(this.technologyName, questionIdsToExclude, 1)
            .pipe(first())
            .subscribe((secondRes) => {
              if (!secondRes.length) {
  ...
  async loadNewQuestion(): Promise<void> {
    this.quizService
      .getAttemptedQuestions(this.getUserId())
      .pipe(first())
      .subscribe(async (res) => {
        if (res) {
          ...
          if (this.totalAttemptedQuestions < this.totalQuestions) {
            this.getQuestion(
              res.map((attemptedQuestion) => attemptedQuestion.questionId)
            );
          } else {
  ...
  async calculateScore(): Promise<void> {
    const attemptedQuestions = await this.quizService
      .getAttemptedQuestions(this.getUserId())
      .toPromise();
  ...
  startTimer(): void {
    ...
    this.timerSubscription = interval(this.TIMER_INTERVAL).subscribe(() => {
      ...
      if (timeRemaining <= 0) {
        ...
        this.timeLeft = `${this.formatTime(minutes)}:${this.formatTime(
          seconds
        )}`;
      }
    });
